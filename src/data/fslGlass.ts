...
export type FslGlassCategory = 'clear' | 'decorative' | 'privacy' | 'clic' | 'blinds'
export type FslGlassOption = { id: string; name: string; category: FslGlassCategory; asset?: string }

const asset = (code: string) => `/assets/hgi-assets/Sidelites/FSL/Glass/FSL${code}.png`
const option = (code: string, name: string, category: FslGlassCategory = 'decorative'): FslGlassOption => ({ id: code.toLowerCase(), name, category, asset: asset(code) })
...
export const fslGlassOptions: FslGlassOption[] = [
  ...
  option('LON', 'London'), option('MAD', 'Madison'), option('MAJ', 'Majestic'), option('MOH', 'Mohave'),
...
export type FslGridRules = Partial<Record<GridPattern, Partial<Record<GridColor, GridWidth[]>>>>
const both: GridWidth[] = ['5/8"', '7/8"']
export const fslStandardFlatRules: FslGridRules = {
...
export const fslLowEFlatRules: FslGridRules = {
...
export const fslStandardStyleRules: Partial<Record<GridStyle, FslGridRules>> = {
...
export const fslLowEStyleRules: Partial<Record<GridStyle, FslGridRules>> = {
  'Arts & Crafts': { '3 Lite': { White: ['5/8"'] } },
  Contoured: { '5 Lite': { Champagne: ['11/16"'], White: ['11/16"'] } },
