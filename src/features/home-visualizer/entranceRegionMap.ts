...
export function buildEntranceRegionMap(width: number, height: number, outer: EntranceCorners, layers: ProductLayer[]) {
  ...
  const layerPolygons = layers.map((layer) => ({
    region: layer.kind === 'frame' ? ENTRANCE_REGION.frame : layer.kind === 'door' ? ENTRANCE_REGION.door : layer.kind === 'left-sidelite' ? ENTRANCE_REGION.leftSidelite : ENTRANCE_REGION.rightSidelite,
