...
export function freeDragWithMagneticSnap(corners:EntranceCorners,id:CornerId,requested:Point,size:{width:number;height:number},state:SnapState,shift=false,extra?:{x?:number[];y?:number[]},touchFriendly=false):{point:Point;state:SnapState;guides:SnapGuides}{
  const index=ORDER.indexOf(id),previous=corners[ORDER[(index+3)%4]],next=corners[ORDER[(index+1)%4]],enter=shift?SHIFT_SNAP_ENTER_DISTANCE_PX:touchFriendly?6:SNAP_ENTER_DISTANCE_PX,release=touchFriendly?8:SNAP_RELEASE_DISTANCE_PX,doubleSnap=touchFriendly?3:DOUBLE_SNAP_DISTANCE_PX
  ...
  const primary=active&&active.distance<=release?active:candidates[0]?.distance<=enter?candidates[0]:undefined
  ...
  const secondary=candidates.find(candidate=>candidate.key!==primary.key&&candidate.distance<=doubleSnap)
  const crossing=secondary&&primary.distance<=doubleSnap?intersection(primary.line,secondary.line):null
  ...
  const useDouble=Boolean(crossing&&crossingDistance<=doubleSnap)
